...
glob.sync( src/less/**/*.less ).forEach(file => {
    ...
    let scssData = data.replace(/\/less\//g,  /scss/ ) // change less/ dir to scss/ on imports
        ...
        .replace(/(\.[\w-] )\s*when\s*\((\$[\w-]*)\s*=\s*(\w )\)\s*({\s*.*?\s*})/gs,  @if ($2 == $3) { $1 $4 } ) // replace conditionals
    ...
    if (filename !==  inverse ) {
        ...
        const joinedHook = `@mixin hook-inverse(){ ${inverseTemplate} } `;
        scssData = scssData.replace(/\*\//,  */     joinedHook);
    }
    ...
    if (filename ===  uikit.theme ) {
        ...
        scssData = scssData.replace(/\/\/ \/\/ Theme \/\/  @import "theme\/_import.scss";/,   );
        /* add uikit-mixins and uikit-variables include to the uikit.scss file and change order, to load theme files first */
        scssData = scssData.replace(/\/\/ Core \/\//g,  // Theme //  @import "theme/_import.scss"; );
    }
...
function getMixinsFromFile(file, data) {
    ...
    return data
        .replace(/@mixin ([\w-]*)\s*\((.*)\)\s*{ (\s [\w\W] ?)(?= }) }/g,   )
        .replace(/@mixin ([\w-]*)\s*\((.*)\)\s*{( [^ ] )}/g,   );
}
