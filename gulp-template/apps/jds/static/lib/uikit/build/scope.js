    ...
    await Promise.all(
        files.map(async store => {
            try {
                ...
                const output = await renderLess(`.${scope} { ${stripComments(store.data)} }`);
                store.data = `/* scoped: ${scope} */ ${
                    output
                        .replace(new RegExp(`.${scope} ${/{(.|[\r ])*?}/.source}`),   )
...
function stripComments(input) {
    return input.replace(/\/\*(.| )*?\*\//gm,   ).split(   ).filter(line => line.trim().substr(0, 2) !==  // ).join(   );
}
