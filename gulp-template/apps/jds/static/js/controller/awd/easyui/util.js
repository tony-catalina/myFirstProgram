define(function(require) {
    ...
    require('easyui');
    require('watermark');
    var layer = require('awd/layui/utils');
    var util = {
        ...
        ajax: function(obj) { //ajax请求数据
            $.ajax({
                ...
                processData: obj.processData == undefined ? true : obj.processData,
                timeout: 30000,
        ...
        ajax请求获取数据
        */
        ajax_ReturnData: function(obj, callback) { //ajax请求数据
            $.ajax({
                url: obj.url,
                async: true, //默认是同步请求
                data: obj.data == undefined ? "" : obj.data,
        ...
        getJson: function(id) {
            var jsonStr;
            $('#' + id).find('tr').each(function() { // id是table表格的id（如果表格有tbody，改为$('#'+id + ' tbody').）
                jsonStr += "{";
