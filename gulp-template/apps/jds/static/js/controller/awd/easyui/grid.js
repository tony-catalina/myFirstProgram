define(function (require) {
    ...
    var main = {
        init: function (id, options) {
            var _data = options['data'] != undefined ? options['data'] : '';
            var _title = options['title'] != undefined ? options['title'] : '';
            var _url = options['url'] != undefined ? options['url'] : (function () {
                utils.alert('url接口为undefined');
            })();
            var _width = options['width'] != undefined ? options['width'] : '100%';
            var _height = options['height'] != undefined ? options['height'] : '100%';
            var _fit = options['fit'] != undefined ? options['fit'] : true;
            var _striped = options['striped'] != undefined ? options['striped'] : false;
            ...
            var _onLoadSuccess = options['onLoadSuccess'] != undefined ? options['onLoadSuccess'] : function () {
                if (_singleSelect) {
                    if ($('#' + id).parent().find('input[name="selectRadio"]')[0]) {
                        $('#' + id).datagrid('selectRow', 0);
                        $('#' + id).parent().find('input[name="selectRadio"]')[0].checked = true;
                    }else{
                        $('#' + id).datagrid('clearSelections');
                    }
                } else {
                    if ($('#' + id).parent().find('input[name="selectCheckBox"]')[0]) {
                        $('#' + id).datagrid('selectRow', 0);
                        $('#' + id).parent().find('input[name="selectCheckBox"]')[0].checked = true;
                    }else{
                        $('#' + id).datagrid('clearSelections');
                    }
                }
            };
            var _onLoadError = options['onLoadError'] != undefined ? options['onLoadError'] : function () {
                utils.alert('数据异常，请稍后再试');
            };
            ...
            var _detailview = options['detailview'] != undefined ? options['detailview'] : this.detailview();
            var _detailviewshow = (options['detailviewshow'] != undefined && options['detailviewshow'] == true) ? true : false;
            var _detailFormatter = options['detailFormatter'] != undefined ? options['detailFormatter'] : function (index, data) { };
            var _onClickRow = options['onClickRow'] != undefined ? options['onClickRow'] : function (rowIndex) {
                if (_singleSelect == true) {
                    $('#' + id).datagrid('clearSelections');
                    if (!tools.isUndefined($('#' + id).parent().find('input[name="selectRadio"]')[rowIndex])) {
                        $('#' + id).parent().find('input[name="selectRadio"]')[rowIndex].checked = true;
                    }
                    $('#' + id).datagrid('selectRow', rowIndex);
                } else {
                    if (!tools.isUndefined($('#' + id).parent().find('input[name="selectCheckBox"]')[rowIndex])) {
                        var selectRow = $('#' + id).parent().find('input[name="selectCheckBox"]')[rowIndex];
            ...
            var _onDblClickCell = options['onDblClickCell'] != undefined ? options['onDblClickCell'] : function () { };
            ...
            var option = {
                ...
                onDblClickCell: _onDblClickCell,
            ...
            if (_detailviewshow) {
                table = $('#' + id).datagrid($.extend({}, option, {
                    view: _detailview,
                    detailFormatter: _detailFormatter,
                }));
            } else {
                table = $('#' + id).datagrid(option);
            }
