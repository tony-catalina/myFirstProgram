    ...
    var main = {
        showWin: function (option, callback) {
            ...
            var laywin = layer.open({
                ...
                maxmin: tools.isUndefined(option.maxmin) ? false : false,
                content: tools.isUndefined(option.content) ? '' : option.content,
                closeBtn: tools.isUndefined(option.closeBtn) ? 0 : 0,
                btnAlign: tools.isUndefined(option.closeBtn) ? "c" : "c",
        ...
        msg: function (msg) {
            layer.message(msg, { icon: 0, shade: 0 });
        }
    };
    return main;
});