define(function(require) {
    ...
    var main = {
        init: function(url,data,callback) {
            var html = formtpl('form/_xzpaSldyj',data);
         if(!isOpen){
        ...
        bindEvent: function() {
            ...
            $("#sldqmrq").datetimebox('setValue',(new Date()).Format('yyyy-MM-dd hh:mm:ss'));

            $('input[type=radio][name=sldyj]').change(function () {
                if (this.value == '1') {
                    $("#sldyjfm").textbox("setValue", "同意");
                } else if (this.value == '2') {
                    $("#sldyjfm").textbox("setValue", "不同意");
                }
            });
        ...
        submit: function(url,data, callback) {
            /**先校验form */
            if ($('#fm').form('validate')) {
                var nowData=(new Date()).Format("yyyy-MM-dd hh:mm:ss");
