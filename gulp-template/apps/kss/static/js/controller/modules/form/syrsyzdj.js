define(function (require) {
    ...
    // 必须引入的对应html文件
    var formtpl = require('services/form/_syrsYzdj.html');
    var popwin;
    var opened = false;
    var main = {
        cache: function () {
            setTimeout(() => {
                ...
                if (popform == '') {
                    var html = formtpl('form/_syrsYzdj', {});
                    $('#formCache').html(html);
                    $.parser.parse('#fm');
                    main.bindEvent();
                    $('#formCache').hide();
        ...
        bindEvent: function () {
            // if($('#zjlx').text=='身份证')
            // 字典
            // $('#zjh').data-options+"=validType:'idcard'"
            ...
            // comboxtree.initTagBox('#fm .awdTagbox', urls.getDictionary);
            ...
            $('#ay').tagbox({
                ...
                onSelect: function (value) {
                    if ($(this).tagbox("getValues").length > 4) {
                        layer.alert('选择不能大于5个', function (index) {
        ...
        submit: function (url, callback) {
            ...
            /** 身份证号码验证 */
            var lxdh = $("#bardh").textbox('getValue');
            // var city = {
            //     11: "北京",
            //     12: "天津",
            //     13: "河北",
            ...
            if (!(/^1[0-9]{10}$/.test(lxdh) || /^(\(\d{3,4}\)|\d{3,4}-|\s)?\d{7,14}$/.test(lxdh))) {
                utils.alert("请输入正确的手机号码或者固定电话")
                $("#bardh").textbox('clear');
                return false;
            }
            /**先校验form */
