 ...
 */
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'jcfs')>-1){
	            clumns.push({field : 'jcfs',title : '检查方式(JCFS)',width : 200,hidden:util.getArrayIndex(option.hidden,'jcfs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jcfs==null?"":row.jcfs)+'>'+(row.jcfs==null?"":row.jcfs)+'</span>'}});
	        }
			if(util.getArrayIndex(option.clumns,'jcfsString')>-1){
				clumns.push({field : 'jcfsString',title : '检查方式',width : 200,hidden:util.getArrayIndex(option.hidden,'jcfsString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jcfsString==null?"":row.jcfsString)+'>'+(row.jcfsString==null?"":row.jcfsString)+'</span>'}});
			}
