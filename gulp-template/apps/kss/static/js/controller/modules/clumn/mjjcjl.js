 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'jclx')>-1){
	            clumns.push({field : 'jclx',title : '奖惩类型',width : 200,hidden:util.getArrayIndex(option.hidden,'jclx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jclx==null?"":row.jclx)+'>'+(row.jclx==null?"":row.jclx)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'jcsj')>-1){
	            clumns.push({field : 'jcsj',title : '奖惩时间',width : 200,hidden:util.getArrayIndex(option.hidden,'jcsj')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jcsj==null?"":row.jcsj)+'>'+(row.jcsj==null?"":row.jcsj)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'jcyy')>-1){
	            clumns.push({field : 'jcyy',title : '奖惩原因',width : 200,hidden:util.getArrayIndex(option.hidden,'jcyy')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jcyy==null?"":row.jcyy)+'>'+(row.jcyy==null?"":row.jcyy)+'</span>'}});
	        }
