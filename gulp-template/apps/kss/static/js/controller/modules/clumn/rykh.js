 ...
 */
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'jkflx')>-1){
	            clumns.push({field : 'jkflx',title : '加扣分类型',width : 200,hidden:util.getArrayIndex(option.hidden,'jkflx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jkflx==null?"":row.jkflx)+'>'+(row.jkflx==null?"":row.jkflx)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'cygljkfs')>-1){
	            clumns.push({field : 'cygljkfs',title : '处于管理考核分',width : 200,hidden:util.getArrayIndex(option.hidden,'cygljkfs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.cygljkfs==null?"":row.cygljkfs)+'>'+(row.cygljkfs==null?"":row.cygljkfs)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'fxpgjkfs')>-1){
	            clumns.push({field : 'fxpgjkfs',title : '风险评估考核分',width : 200,hidden:util.getArrayIndex(option.hidden,'fxpgjkfs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.fxpgjkfs==null?"":row.fxpgjkfs)+'>'+(row.fxpgjkfs==null?"":row.fxpgjkfs)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'sftg')>-1){
	            clumns.push({field : 'sftg',title : '是否通过',width : 200,hidden:util.getArrayIndex(option.hidden,'sftg')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.sftg==null?"":row.sftg)+'>'+(row.sftg==null?"":row.sftg)+'</span>'}});
	        }
