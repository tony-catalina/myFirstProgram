...
define(function (require) {
    ...
    var main = {
      init: function () {
        ywlcCount.init("kss_hjbd", function (data) {
          // main.layout(data);
        });
        var api = "/plugin-flowchart/scene/simulation.json";
        var firstLoadFlow = "环节变动";
      ...
      generalScene: function (element, api, flowConfig) {
        ...
        ywlcCount.init("kss_hjbd", function (data) {
          ...
          config[0].data[0].extData.events = data.KSS_HJBD_QS;
          config[0].data[1].extData.events = data.KSS_HJBD_HZ;
          ...
          __scene_flow = new vEngine.VEngine({
            ...
            on: {
              click: function (target, e) {
                ...
                if (target[0].case.options.name == "办案环节变动" || target[0].case.options.text == "办案环节变动") {
                  window.parent.parent.openTab('办案环节变动', '/apps/kss/views/pages/hjbd/hjbd.html');
                }
