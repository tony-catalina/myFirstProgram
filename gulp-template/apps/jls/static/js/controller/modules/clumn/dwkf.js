 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'kfsj')>-1){
	            clumns.push({field : 'kfsj',title : '开放时间',width : 200,hidden:util.getArrayIndex(option.hidden,'kfsj')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.kfsj==null?"":row.kfsj)+'>'+(row.kfsj==null?"":row.kfsj)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'kflx')>-1){
	            clumns.push({field : 'kflx',title : '开放类型',width : 200,hidden:util.getArrayIndex(option.hidden,'kflx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.kflx==null?"":row.kflx)+'>'+(row.kflx==null?"":row.kflx)+'</span>'}});
	        }
			if(util.getArrayIndex(option.clumns,'kflxString')>-1){
			    clumns.push({field : 'kflxString',title : '开放类型',width : 200,hidden:util.getArrayIndex(option.hidden,'kflxString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.kflxString==null?"":row.kflxString)+'>'+(row.kflxString==null?"":row.kflxString)+'</span>'}});
			}
	        if(util.getArrayIndex(option.clumns,'cgdx')>-1){
	            clumns.push({field : 'cgdx',title : '参观对象',width : 200,hidden:util.getArrayIndex(option.hidden,'cgdx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.cgdx==null?"":row.cgdx)+'>'+(row.cgdx==null?"":row.cgdx)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'cgrs')>-1){
	            clumns.push({field : 'cgrs',title : '参观人数',width : 200,hidden:util.getArrayIndex(option.hidden,'cgrs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.cgrs==null?"":row.cgrs)+'>'+(row.cgrs==null?"":row.cgrs)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'sqly')>-1){
	            clumns.push({field : 'sqly',title : '申请理由',width : 200,hidden:util.getArrayIndex(option.hidden,'sqly')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.sqly==null?"":row.sqly)+'>'+(row.sqly==null?"":row.sqly)+'</span>'}});
	        }
			...
			if(util.getArrayIndex(option.clumns,'szyjString')>-1){
				clumns.push({field : 'szyjString',title : '所长意见',width : 200,hidden:util.getArrayIndex(option.hidden,'szyjString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.szyjString==null?"":row.szyjString)+'>'+(row.szyjString==null?"":row.szyjString)+'</span>'}});
			}
			...
			if(util.getArrayIndex(option.clumns,'sjldyjString')>-1){
				clumns.push({field : 'sjldyjString',title : '上级领导意见',width : 200,hidden:util.getArrayIndex(option.hidden,'sjldyjString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.sjldyjString==null?"":row.sjldyjString)+'>'+(row.sjldyjString==null?"":row.sjldyjString)+'</span>'}});
			}
