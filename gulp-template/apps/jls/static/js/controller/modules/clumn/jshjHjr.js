 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'uuid')>-1){
	            clumns.push({field : 'uuid',title : '家属会见',width : 200,hidden:util.getArrayIndex(option.hidden,'uuid')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.uuid==null?"":row.uuid)+'>'+(row.uuid==null?"":row.uuid)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'zz')>-1){
	            clumns.push({field : 'zz',title : '住址',width : 200,hidden:util.getArrayIndex(option.hidden,'zz')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.zz==null?"":row.zz)+'>'+(row.zz==null?"":row.zz)+'</span>'}});
	        }
