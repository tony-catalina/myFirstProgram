 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'lsxm')>-1){
	            clumns.push({field : 'lsxm',title : '律师/代理人姓名',width : 200,hidden:util.getArrayIndex(option.hidden,'lsxm')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.lsxm==null?"":row.lsxm)+'>'+(row.lsxm==null?"":row.lsxm)+'</span>'}});
	        }
			...
			if(util.getArrayIndex(option.clumns,'dwString')>-1){
	            clumns.push({field : 'dwString',title : '律师/代理人单位',width : 200,hidden:util.getArrayIndex(option.hidden,'dwString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.dwString==null?"":row.dwString)+'>'+(row.dwString==null?"":row.dwString)+'</span>'}});
	        }
