 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'jsnbx')>-1){
	            clumns.push({field : 'jsnbx',title : '拘所内表现',width : 200,hidden:util.getArrayIndex(option.hidden,'jsnbx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jsnbx==null?"":row.jsnbx)+'>'+(row.jsnbx==null?"":row.jsnbx)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'mxfzqx')>-1){
	            clumns.push({field : 'mxfzqx',title : '明显犯罪倾向',width : 200,hidden:util.getArrayIndex(option.hidden,'mxfzqx')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.mxfzqx==null?"":row.mxfzqx)+'>'+(row.mxfzqx==null?"":row.mxfzqx)+'</span>'}});
	        }
