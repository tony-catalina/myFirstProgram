 ...
 */
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'rjyy')>-1){
	            clumns.push({field : 'rjyy',title : '入拘原因',width : 200,hidden:util.getArrayIndex(option.hidden,'rjyy')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.rjyy==null?"":row.rjyy)+'>'+(row.rjyy==null?"":row.rjyy)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'wfxf')>-1){
	            clumns.push({field : 'wfxf',title : '违法行为',width : 200,hidden:util.getArrayIndex(option.hidden,'wfxf')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.wfxf==null?"":row.wfxf)+'>'+(row.wfxf==null?"":row.wfxf)+'</span>'}});
	        }
