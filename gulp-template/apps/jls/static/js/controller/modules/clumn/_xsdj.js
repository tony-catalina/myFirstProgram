 ...
 define(function (require) {
	    ...
	    var main=function(option){
			...
			if(util.getArrayIndex(option.clumns,'xslxString')>-1){
			    clumns.push({field : 'xslxString',title : '线索类型',width : 200,hidden:util.getArrayIndex(option.hidden,'xslxString')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.xslxString==null?"":row.xslxString)+'>'+(row.xslxString==null?"":row.xslxString)+'</span>'}});
			}
	        if(util.getArrayIndex(option.clumns,'xslr')>-1){
	            clumns.push({field : 'xslr',title : '线索内容',width : 200,hidden:util.getArrayIndex(option.hidden,'xslr')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.xslr==null?"":row.xslr)+'>'+(row.xslr==null?"":row.xslr)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'jjrq')>-1){
	            clumns.push({field : 'jjrq',title : '坦白/检举日期',width : 200,hidden:util.getArrayIndex(option.hidden,'jjrq')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jjrq==null?"":row.jjrq)+'>'+(row.jjrq==null?"":row.jjrq)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'zajurs')>-1){
	            clumns.push({field : 'zajurs',title : '治安拘留人数',width : 200,hidden:util.getArrayIndex(option.hidden,'zajurs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.zajurs==null?"":row.zajurs)+'>'+(row.zajurs==null?"":row.zajurs)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'dbrs')>-1){
	            clumns.push({field : 'dbrs',title : '带逮捕人数',width : 200,hidden:util.getArrayIndex(option.hidden,'dbrs')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.dbrs==null?"":row.dbrs)+'>'+(row.dbrs==null?"":row.dbrs)+'</span>'}});
	        }
