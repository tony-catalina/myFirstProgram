 ...
 define(function (require) {
	    ...
	    var main=function(option){
	        ...
	        if(util.getArrayIndex(option.clumns,'jzyj')>-1){
	            clumns.push({field : 'jzyj',title : '解除警长意见',width : 200,hidden:util.getArrayIndex(option.hidden,'jzyj')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.jzyj==null?"":row.jzyj)+'>'+(row.jzyj==null?"":row.jzyj)+'</span>'}});
	        }
	        ...
	        if(util.getArrayIndex(option.clumns,'pastable')>-1){
	            clumns.push({field : 'pastable',title : '是否失效',width : 200,hidden:util.getArrayIndex(option.hidden,'pastable')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.pastable==null?"":row.pastable)+'>'+(row.pastable==null?"":row.pastable)+'</span>'}});
	        }
	        if(util.getArrayIndex(option.clumns,'ywlcsyid')>-1){
	            clumns.push({field : 'ywlcsyid',title : '业务流程使用ID',width : 200,hidden:util.getArrayIndex(option.hidden,'ywlcsyid')>-1?true:false,sortable:'true',formatter: function(value,row,index){return '<span title='+(row.ywlcsyid==null?"":row.ywlcsyid)+'>'+(row.ywlcsyid==null?"":row.ywlcsyid)+'</span>'}});
	        }
