define(function(require, exports, module) {
        ...
        if (baseData.navList.indexOf(name) === -1) {
            ...
            if (name === '首页')
                ...
                el.append('<div class="swiper-slide r-nav-child dper-por dper-full-h select-nav" style="min-width:200px;">' +
                    '<div class="malign-center inside-box " style="height:80%;">' +
                    '   <span class="malign-center-text font-20 text-overflow-e" style="display:flex;justify-content:center;">' + res +
                    '</span><i class="close malign-center-right" data-name="' + name + '" data-url="' + url + '">✗</i></div>' + '</div>');
            new swiper('#Swiper', {
                freeMode: true,
    ...
    var event = function(role) {
        handleNavClickFn();
        handleCreateNav('首页', dashboard = '/apps/jls/views/dashboard.html');
    };
    ...
    var initData = function(role) {
        $.get(url.getMenus + '?role=' + role, function(data) {
            ...
            // 左边栏title对应的icon标签
            var iconfontArray = [
                { title: '出入所', icon: 'icon-ico_qiantaijiedai_banlituifang' },
                { title: '窗口服务', icon: 'icon-chuangkoufuwu' },
                { title: '信息维护', icon: 'icon-guanjiaogangwei' },
                { title: '值班管理', icon: 'icon-yewuguanli' },
                { title: '拘室调整', icon: 'icon-yiliaowenjian' },
                { title: '医疗卫生', icon: 'icon-renlian' },
                { title: '当日动态', icon: 'icon-xunkong' },
                { title: '安全管理', icon: 'icon-finance' },
                { title: '统计查询', icon: 'icon-suolingdao' },
                { title: '数据质量', icon: 'icon-chaxuntongji' },
                { title: '档案管理', icon: 'icon-chuangkoufuwu' },
                { title: '队伍管理', icon: 'icon-finance' },
                { title: '查询统计', icon: 'icon-zhaopian' }
            ];
