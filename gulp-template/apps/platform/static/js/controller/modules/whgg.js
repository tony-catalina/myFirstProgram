define(function (require) {
    ...
    var tpl = require('services/whgg_form.html');
    ...
    var main = {
        list: function (usertype) {
            util.ajax({
                ...
                func: function (result) {
                    ...
                    $('#whgglist').html(html);
                    $('#whgglist .b-item').addClass("whgg");
                    $('.b-item.whgg').on('click', function () {
                        var id = $(this).attr('id');
                        window.open('/apps/platform/views/xq.html?mode=whgg&id=' + id);
                    });
                }
            });
        },
        ...
        update: function (datas, refresh) {
            ...
            if (!isOpen) {
                var html = tpl('whgg_form', datas[0]);
                popwin = utils.showWin({
                    area: ['100%', '100%'],
                    content: html,
                    buttons: ['保存', '取消'],
