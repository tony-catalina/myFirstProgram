define(function(require) {
	...
	var tpl = require('services/xwyw_form.html');
	...
	var main = {
		list: function(usertype) {
			util.ajax({
				...
				func: function(result) {
					...
					$('#xwywlist').html(html);
					$('#xwywlist .b-item').addClass("xwyw");
					$('.b-item.xwyw').on('click', function() {
						var id = $(this).attr('id');
						window.open('/apps/platform/views/xq.html?mode=xwyw&id=' + id);
					});
				}
			});
		},
		...
		update: function(datas, refresh) {
			...
			if (!isOpen) {
				var html = tpl('xwyw_form', datas[0]);
				popwin = utils.showWin({
					area: ['100%', '100%'],
					content: html,
					buttons: ['保存', '取消'],
