define(function (require) {
	...
	var tpl = require('services/flgl_form.html');
	...
	var main = {
		...
		update: function (datas, refresh) {
			...
			if (!isOpen) {
				var html = tpl('flgl_form', datas[0]);
				popwin = utils.showWin({
					area: ['100%', '100%'],
					content: html,
					buttons: ['保存', '取消'],
		...
		del: function (datas, refresh) {
			...
			// 	}
			// 	console.log(ids)
			// 	$.messager.confirm('提示', '确认要删除吗？', function(index) {
			// 		util.ajax({
			// 			url: urls.deleteClassfic,
			// 			data: {
			// 				"ids": ids
			// 			},
			// 			func: function(data) {
